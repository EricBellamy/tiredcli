...
function getLastWatch() {
	let watchFile;
	try {
		watchFile = JSON.parse(global.tired.html.files.read("watch.json").toString());
...
function processWatch(watchCallback) {
	global.tired.html.status = "watching";
	...
	const currentWatch = getModifiedDates(fileQueue, global.tired.help.clone(lastWatch));
	...
	watchCallback(changedFiles, function () {
		...
		global.tired.html.files.saveJson("watch.json", currentWatch);

		// If during the build, files were changed, process again immediately
		if (queue.length != 0) processWatch(watchCallback);
		else global.tired.html.status = false;
	});
}

const watchDelay = 100;
module.exports = function (watchCallback) {
	const processFunc = global.tired.help.debounce(processWatch.bind(null, watchCallback), watchDelay);
	...
	fs.watch('.', {
		...
		if (pathParts[0] != global.tired.location.folder) {

			if (!queue.includes(filename)) queue.push(filename);

			// Process if not already processing
			if (global.tired.html.status === false) processFunc();
