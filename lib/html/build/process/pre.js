...
async function loadDefaults(htmlTag) {
	if (loaded.status === false) {
		...
		const fileBase = 'lib/html/build/process/pre';
		...
		for (const file_path in cssFiles) loaded.css += `<style>${await tired.help.readLinkFile(`${tired.location.root}/${fileBase}/css/${file_path}`)}</style>`;
		for (const file_path in jsFiles) loaded.js += `<script>${await tired.help.readLinkFile(`${tired.location.root}/${fileBase}/js/${file_path}`)}</script>`;
	}
