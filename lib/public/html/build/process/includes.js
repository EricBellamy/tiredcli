...
module.exports = {
	...
	processDocument: async function (key, document, data = {}, isAfterValidation = false) {
		...
		for (const node of nodes) {
			...
			included.push(tired.html.help.formatIncludeSrc(src, filetype));

			// Process the file
			let contents;
			let includeSrc = `include/${filetype}/${src}`;
			try {
				...
				const processResult = await link.process(document, node, contents);
				if (processResult.export) exported.push(includeSrc);
			} catch (err) {
				switch (err.code) {
					case "ENOENT":
						tired.log.color(['yellow', '[includes.js/processDocument] Include "'],
							['magenta', includeSrc],
						...
						break;
					default:
						tired.log.color(['red', '[includes.js/processDocument] Unhandled include error "'],
							['magenta', err.message],
							['yellow', '"']);
						break;
				}
