...
module.exports = {
	...
	onload: async function (contents, attributes) {
		...
		for (loop of loops) {
			...
			let iterables;
			if(loop.iterable.indexOf(".") != -1) {
				const iterable = loop.iterable.split(".");
				if(iterable[0] === "template") iterables = templateInputData[iterable[1]];
				else iterables = attributes[iterable[0]][iterable[1]];
			}
