module.exports = {
	...
	load: async function (type) {
		...
		const typeFolderKeys = tired.root.files.readDirs(`private/${type}`, [], false, '');
		...
		for (const typeFolderKey in typeFolderKeys) {
			...
			const projectFolderBaseFiles = tired.root.files.readDirFiles.forType(`private/${type}/${typeFolderKey}`, 'js', true, '');
			for (const baseFileKey in projectFolderBaseFiles) {
				...
				let baseFileContents = await tired.root.files.readText(`private/${type}/${typeFolderKey}/${baseFileKey}`);
				if (baseFileContents.indexOf('module.exports') != -1 && baseFileContents.substring(baseFileContents.indexOf('module.exports')).indexOf(`${type}:`) != -1) {
					...
					if (baseFile && baseFile[type] != undefined) {
						...
						// Set the main key
						if (this.libraries[type][typeFolderKey].main === undefined || (baseFileKey === 'index.js' || baseFileKey === 'main.js')) this.libraries[type][typeFolderKey].main = baseFileKey;

						// Set the module
						this.libraries[type][typeFolderKey].modules[baseFileKey] = {
							key: baseFileKey,
							require: baseFile,
							executions: 0
						}
