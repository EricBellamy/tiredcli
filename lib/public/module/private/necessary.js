...
module.exports.processLibrary = async function (library, document) {
	...
	for (let necessaryScriptPath of necessaryScripts) {
		...
		nestedHeadScripts += await tired.help.readLinkFile(necessaryScriptPath, {}, true);
	}

	// Prepend scripts to head
	const headElement = document.querySelector("head");
	headElement.innerHTML = `<script>window.tired={};${nestedHeadScripts}</script>` + headElement.innerHTML;
}