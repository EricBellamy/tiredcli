...
tired.files.stat = function (filename) { return fs.statSync(`${tired.location.folder}/${filename}`); }
tired.files.ensureDir = function (filename) { return fs.ensureDirSync(`${tired.location.folder}/${filename}`); }
tired.files.readJson = function (filename, throwError = true) { return tired.help.readJson(tired.location.folder + "/" + filename, throwError); }
...
tired.help.readJson = tired.root.require('lib/public/module/files/readJson.js');
...
tired.private.env = function(key, value){
	if (value) tired.private.env.data[key] = value;
	else {
		const envValue = tired.private.env.data[key];
		return envValue != undefined ? tired.private.env.data[key] : false;
	}
}
tired.private.env.data = {};
tired.private.env.clear = function(key){
	delete tired.private.env.data[key];
}

// Watch
tired.watch = {
	ignore: [],
	ignoreNext: function(fileString){
		this.ignore.push(fileString);
	}
}